    ...
    def detect_technologies(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                url,
                headers=(
                    custom_headers if custom_headers else {"User-Agent": "Mozilla/5.0"}
                ),
                timeout=30,
            )
            ...
            if "Vercel" in server_header or any(
                h.startswith("x-vercel-") for h in response.headers.keys()
            ):
                technologies["cdn"] = "Vercel"
            elif "Cloudflare" in server_header or "cf-ray" in response.headers:
                technologies["cdn"] = "Cloudflare"
            elif "Fastly" in server_header or "Fastly" in response.headers.get(
                "Via", ""
            ):
            ...
            logger.info(
                f"Wappalyzer detection completed: Server: {technologies['server']}, "
    ...
    def _process_technology(self, tech_name, tech_info, technologies):
        ...
        if categories:
            for category in categories:
                cat_name = (
                    category if isinstance(category, str) else category.get("name", "")
                )
