...
def check_hsts_header(scan, headers):
    ...
    if "max-age=" in hsts_header:
        try:
            max_age_part = [
                part.strip() for part in hsts_header.split(";") if "max-age=" in part
            ][0]
...
def _analyze_headers(self):
    ...
    try:
        ...
        from scanning.passive.analyzers.header_analyzer import \
...
def check_header_information_disclosure(scan, headers):
    ...
    for header, value in headers.items():
        if header.lower().startswith(
            ("x-aspnet", "x-iis", "x-weblogic", "x-websphere", "x-oracle")
        ):
    ...
    if sensitive_info:
        ...
        logger.info(
            f"Header information disclosure detected: {'; '.join(sensitive_info)}"
        )
