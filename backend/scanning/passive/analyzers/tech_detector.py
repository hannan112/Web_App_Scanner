...
def detect_technologies(response_text, response_headers):
    ...
    technologies["javascript_libraries"] = list(
        set(technologies["javascript_libraries"])
    )
    technologies["programming_languages"] = list(
        set(technologies["programming_languages"])
    )
...
def detect_cms(soup, response_text, response_headers, technologies):
    ...
    if any(
        ...
        for term in ["wp-content", "wp-includes", "wordpress"]
    ):
    ...
    # Drupal detection
        ...
        term in response_text.lower()
        for term in ["drupal.settings", "drupal.org", "/sites/all/themes/"]
    ):
    ...
    # Joomla detection
        ...
        term in response_text.lower()
        for term in ["joomla!", "/components/com_", "/media/jui/"]
    ):
    ...
    # Magento detection
        ...
        term in response_text.lower()
        for term in ["magento", "mage/cookies.js", "enterprise_cms"]
    ):
        technologies["cms"] = "Magento"

    # Shopify detection
    elif any(
        term in response_text.lower() for term in ["shopify.com", "/cdn.shopify.com/"]
    ):
        technologies["cms"] = "Shopify"

    # Wix detection
    elif any(
        term in response_text.lower() for term in ["wix.com", "_wixCssrules", "_wixads"]
    ):
    ...
    # Squarespace detection
        ...
        term in response_text.lower()
        for term in ["squarespace.com", "static.squarespace.com"]
    ):
...
def detect_frameworks(soup, response_text, response_headers, technologies):
    ...
    # Django detection
    if (
        "csrfmiddlewaretoken" in response_text
        or "__admin_media_prefix__" in response_text
    ):
    ...
    if "rails" in response_text.lower() or "_rails_env" in response_text:
        ...
        "x-powered-by" in response_headers
        and "rails" in response_headers.get("x-powered-by", "").lower()
    ):
        technologies["frameworks"].append("Ruby on Rails")
    ...
    if "asp.net" in response_text.lower() or "__VIEWSTATE" in response_text:
        ...
        technologies["frameworks"].append(
            f"ASP.NET {response_headers['x-aspnet-version']}"
        )
...
def detect_js_libraries(soup, response_text, technologies):
    ...
    for script in soup.find_all("script"):
        if script.string:
            ...
            if (
                "jquery" in script_content
                and "jQuery" not in technologies["javascript_libraries"]
            ):
                technologies["javascript_libraries"].append("jQuery")

            if (
                "react" in script_content or "reactdom" in script_content
            ) and "React" not in technologies["frameworks"]:
                technologies["frameworks"].append("React")
    ...
    for link in soup.find_all("link", rel="stylesheet"):
        ...
        if (
            "bootstrap" in href
            and "Bootstrap" not in technologies["javascript_libraries"]
        ):
            technologies["javascript_libraries"].append("Bootstrap")

        if (
            "tailwind" in href
            and "Tailwind CSS" not in technologies["javascript_libraries"]
        ):
            technologies["javascript_libraries"].append("Tailwind CSS")
...
def detect_programming_languages(response_headers, technologies):
    ...
    if "php" in response_headers.get("x-powered-by", "").lower():
        ...
        php_version_match = re.search(
            r"PHP/(\d+\.\d+\.\d+)", response_headers.get("x-powered-by", "")
        )
        if php_version_match:
            technologies["programming_languages"][
                -1
            ] = f"PHP {php_version_match.group(1)}"
    ...
    if any(
        java_term in response_headers.get("server", "").lower()
        for java_term in ["tomcat", "jetty", "jboss", "websphere"]
    ):
        technologies["programming_languages"].append("Java")

    # Ruby detection
    if (
        "phusion passenger" in response_headers.get("server", "").lower()
        or "ruby" in response_headers.get("x-powered-by", "").lower()
    ):
        technologies["programming_languages"].append("Ruby")

    # Python detection
    if (
        "python" in response_headers.get("x-powered-by", "").lower()
        or "wsgi" in response_headers.get("server", "").lower()
    ):
...
def detect_web_servers(response_headers, technologies):
    ...
    if "apache" in server_header:
        ...
        if apache_version_match:
            technologies["web_servers"].append(
                f"Apache {apache_version_match.group(1)}"
            )
        else:
    ...
    # Microsoft IIS detection
        ...
        if iis_version_match:
            technologies["web_servers"].append(
                f"Microsoft IIS {iis_version_match.group(1)}"
            )
        else:
...
def detect_analytics(soup, response_text, technologies):
    ...
    if (
        "google-analytics.com" in response_text
        or "googletagmanager.com" in response_text
    ):
...
def detect_cdn(response_headers, technologies):
    ...
    if (
        "cf-ray" in response_headers
        or "cloudflare" in response_headers.get("server", "").lower()
    ):
    ...
    # Cloudfront detection
        ...
        "cloudfront" in response_headers.get("via", "").lower()
        or "x-amz-cf-id" in response_headers
    ):
