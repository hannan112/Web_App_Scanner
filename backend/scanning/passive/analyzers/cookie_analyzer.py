...
def analyze_cookies(scan, cookies, headers=None):
    ...
    create_cookie_vulnerability_reports(
        scan, insecure_cookies, httponly_missing, samesite_missing, sensitive_cookies
    )


def create_cookie_vulnerability_reports(
    scan, insecure_cookies, httponly_missing, samesite_missing, sensitive_cookies
):
    """
    ...
    session_cookies = [
        cookie for cookie in sensitive_cookies if "sess" in cookie.lower()
    ]
    if session_cookies:
        # Check if any of the session cookies are also in the insecure or httponly_missing lists
        session_insecure = [
            cookie for cookie in session_cookies if cookie in insecure_cookies
        ]
        session_not_httponly = [
            cookie for cookie in session_cookies if cookie in httponly_missing
        ]
        ...
        if session_insecure or session_not_httponly:
            ...
            logger.info(
                f"Found insecure session cookies: {', '.join(set(session_insecure + session_not_httponly))}"
            )
...
def check_sequential_patterns(cookies):
    ...
    for name, value in cookies.items():
        if "sess" in name.lower() or "id" in name.lower():
            ...
            ):  # Short numeric values are suspicious
