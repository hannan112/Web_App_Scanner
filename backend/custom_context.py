...
def should_exclude_file(file_name):
    excluded_exts = ['.env', '.db', '.sqlite', '.sqlite3', '.sql3', '.db3', '.pyc', '.so', '.dll', '.exe', '.bin']
        ...
        for module in modules_to_include:
            ...
            for root, dirs, files in os.walk(module_path):
                ...
                for file in sorted(files):
                    if should_exclude_file(file) or is_binary_file(os.path.join(root, file)):
                        continue
            ...
            for root, _, files in os.walk(module_path):
                for file in sorted(files):
                    ...
                    if should_exclude_file(file) or is_binary_file(file_path) or not is_code_file(file):
                        continue
...
def prompt_user_choice():
    ...
    if choice == "1":
        ...
        modules_input = input("Enter module names separated by spaces (e.g., scanning auth): ").strip()
...
if __name__ == "__main__":
    ...
    if mode == "all":
        ...
        modules_to_include = [
            ...
            if os.path.isdir(os.path.join(base_dir, d)) and d not in ["venv", ".git", "__pycache__", "node_modules", ".next"]
        ]
