...
class ScanTracker:
    ...
    def stop_scan(self, scan_id: int) -> bool:
        ...
        # Critical section: only get the engine reference
        with self._lock:
            engine = self._running_scans.get(scan_id)
            
        if engine:
            try:
                # Stop the scan engine OUTSIDE the lock
                # This prevents deadlock when engine.stop_scan() calls unregister_scan()
                # which needs to acquire the lock again.
                # Even with RLock, it's better to not hold the lock during long operations.
