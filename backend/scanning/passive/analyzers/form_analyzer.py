...
def analyze_forms(scan, forms):
    ...
    for form in forms:
        ...
        for input_field in form.get("inputs", []):
            ...
            if any(
                csrf_term in field_name
                for csrf_term in ["csrf", "token", "nonce", "xsrf"]
            ):
                has_csrf = True
                break
        ...
        for input_field in form.get("inputs", []):
            ...
            if field_type == "password":
                ...
                if field_autocomplete != "off" and field_autocomplete != "new-password":
                    autocomplete_enabled.append(
                        form_url or form_action or "Unknown form URL"
                    )
    ...
    for form_url in password_inputs:
        for form in forms:
            if (
                form.get("url") == form_url or form.get("action") == form_url
            ) and form.get("method", "").upper() != "POST":
...
def check_login_form_security(scan, forms):
    ...
    for form in forms:
        ...
        has_password = any(
            input_field.get("type") == "password" for input_field in inputs
        )
        has_username = any(
            any(
                user_field in input_field.get("name", "").lower()
                for user_field in ["user", "email", "login", "name"]
            )
            for input_field in inputs
        )

        login_keywords = ["login", "signin", "sign-in", "auth", "account"]
        url_suggests_login = any(
            keyword in (form_url.lower() + form_action.lower())
...
def check_brute_force_protection(scan, login_forms):
    ...
    for form in login_forms:
        ...
        if form_html and any(
            captcha_term in form_html.lower()
            for captcha_term in ["captcha", "recaptcha", "g-recaptcha", "cf-turnstile"]
        ):
            has_protection = True
        ...
        if not has_protection:
            forms_without_protection.append(
                form_url or form_action or "Unknown form URL"
            )
    ...
    if forms_without_protection and len(forms_without_protection) == len(login_forms):
        ...
        logger.info(
            f"Found {len(forms_without_protection)} login forms without apparent brute force protection"
        )
