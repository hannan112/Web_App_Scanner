...
from scanning.models import (CrawlResult, PassiveReconResult, Scan,
                             ScanConfiguration, ScanLog, Vulnerability)

from .serializers import (CrawlResultSerializer, PassiveReconResultSerializer,
...
class ScanViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        configuration = get_object_or_404(
            ScanConfiguration, id=config_id, project=project
        )

        # Create scan
        scan = Scan.objects.create(
            project=project, configuration=configuration, status="pending"
        )
