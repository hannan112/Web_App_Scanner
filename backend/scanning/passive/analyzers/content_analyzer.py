...
def check_html_comments(scan, url, soup):
    ...
    if sensitive_comments:
        ...
        logger.info(
            f"Found {len(sensitive_comments)} potentially sensitive HTML comments on {url}"
        )
...
def check_exposed_emails(scan, url, html_content):
    ...
    if emails:
        ...
        sensitive_emails = [
            email for email in emails if not any(ns in email for ns in non_sensitive)
        ]
        ...
        if sensitive_emails:
            ...
            logger.info(
                f"Found {len(sensitive_emails)} exposed email addresses on {url}"
            )
...
def check_version_disclosure(scan, url, html_content, headers):
    ...
    if server_header and any(
        tech in server_header.lower()
        for tech in ["apache", "nginx", "iis", "php", "tomcat"]
    ):
